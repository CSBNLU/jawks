...
export const compose = <AccessTokenPayload, RefreshTokenPayload>(
  ...
  const { accessTokenPrivateKeyRepository, refreshTokenPrivateKeyRepository } =
  ...
  const jwtModule: JWT.API.JWTModule<AccessTokenPayload, RefreshTokenPayload> =
    JWT.Bindings.create({
  ...
  const { accessTokenAuthorizer, refreshTokenAuthorizer } =
