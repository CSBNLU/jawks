...
export const create: <AccessTokenPayload, RefreshTokenPayload>(
  ...
  const authorizerFunction: aws.lambda.Function = new aws.lambda.CallbackFunction(
    `${props.prefix}-access-token-authorizer`,
    {
      architectures: ["arm64"],
      callback: lambdaAuthorizer.create({
        authorizer: deps.jwtModule.accessTokenAuthorizer,
      }),
    },
  );
